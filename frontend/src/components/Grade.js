...
const Grade = (props) => {
  ...
  let grades = async () => {
    return await fetch(`http://localhost:3000/grade/${props.match.params.id}`);
  }

  const { name, subject, type, value } = grades;
  ...
  const updateGrade = () => {
    ...
    GradeDataService.update(props.match.params.id, currentGrade)
      .then((response) => {
